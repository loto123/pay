        ...
        <div class="box-header">
            <form class="form-horizontal" method="post" action="/admin/agent_card/card_record">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-2">
                        <input type="text" name="allocate_id" id="allocate_id" class="form-control" placeholder="请输入卡号"
                               value="{{$allocate_id??''}}">
                    </div>

                    <label class="col-sm-1 control-label">用卡人ID：</label>
                    <div class="col-sm-2">
                        <input type="text" name="operator_id" id="operator_id" class="form-control" placeholder="请输入用户ID"
                               value="{{$operator_id??''}}">
                    </div>

                    <label class="col-sm-1 control-label">运营ID：</label>
                    <div class="col-sm-2">
                        <input type="text" name="promoter_id" id="promoter_id" class="form-control" placeholder="请输入运营ID"
                               value="{{$promoter_id??''}}">
                    </div>

                    <label class="col-sm-1 control-label">推广员ID：</label>
                    <div class="col-sm-2">
                        <input type="text" name="card_id" id="card_id" class="form-control" placeholder="请输入推广员ID"
                               value="{{$card_id??''}}">
                    </div>
                    <label class="col-sm-1 control-label">使用状态：</label>
                    <div class="col-sm-2">
                            <select name="使用状态" id="is_bound">
                                <option value="">所有</option>
                                <option value="{{\App\Agent\Card::BOUND}}">已使用</option>
                                <option value="{{\App\Agent\Card::UNBOUND}}">未使用</option>
                            </select>
                    </div>
                    <label class="col-sm-1 control-label">是否冻结：</label>
                    <div class="col-sm-2">
                        <select name="冻结状态" id="is_bound">
                            <option value="">所有</option>
                            <option value="{{\App\Agent\Card::FROZEN}}">已冻结</option>
                            <option value="{{\App\Agent\Card::UNFROZEN}}">未冻结</option>
                        </select>
                    </div>
                </div>
                        ...
                        <tr>
                            ...
                            <td>{{$item->owner}}
                                {{--<div class="user-panel clearfix">--}}
                                    {{--<div class="pull-left">--}}
                                        {{--<img src="{{$item->owner['avatar']}}" width="40" height="40" class="img-circle">--}}
                                    {{--</div>--}}
                                    {{--<div class="pull-left ml7">--}}
                                        {{--<p>{{$item->owner['name']}}</p>--}}
                                        {{--<span>ID:<span>{{$item->owner['username']}}</span></span>--}}
                                    {{--</div>--}}
                                {{--</div>--}}
                            </td>
                            <td>{{$item->promoters}}
